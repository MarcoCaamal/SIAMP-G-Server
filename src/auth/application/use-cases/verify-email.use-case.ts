...
export class VerifyEmailUseCase {
  ...
  async execute(verifyEmailDto: VerifyEmailDto): Promise<{ message: string }> {
    ...
    const user = await this.authRepository.findUserByVerificationToken(verifyEmailDto.token);
    if (!user) {
      throw new Error('Invalid or expired verification token');
    }

    // Check if token is expired
    if (user.verificationToken && new Date() > user.verificationToken.expiresAt) {
